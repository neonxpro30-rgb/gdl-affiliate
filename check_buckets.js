const admin = require('firebase-admin');

const serviceAccount = {
    projectId: "gdl-database-b32f4",
    clientEmail: "firebase-adminsdk-fbsvc@gdl-database-b32f4.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
};

admin.initializeApp({
    credential: admin.credential.cert(serviceAccount)
});

async function listBuckets() {
    try {
        // Access the underlying Google Cloud Storage client
        const storage = admin.storage();
        // We need to get the storage client. 
        // A hacky way is to get a bucket reference and access its storage property, 
        // or just try to use the default bucket if it exists, but we don't know it.
        // Actually, let's try to import Storage from @google-cloud/storage if possible, 
        // but we can't easily require it if it's not explicitly installed in package.json (though it is a dependency).

        // Better approach:
        const bucket = storage.bucket('dummy-bucket');
        const [buckets] = await bucket.storage.getBuckets();

        console.log('Buckets:');
        if (buckets.length === 0) {
            console.log('No buckets found.');
        } else {
            buckets.forEach(bucket => {
                console.log(bucket.name);
            });
        }
    } catch (err) {
        console.error('ERROR:', err);
    }
}

listBuckets();
