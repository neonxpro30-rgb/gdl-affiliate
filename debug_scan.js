...
async function debugScan() {
    console.log('Fetching ALL Orders & Sorting in Memory...');

    const ordersSnap = await db.collection('orders').get(); // Fetch ALL
    if (ordersSnap.empty) {
    ...
    // Checking Commission
    const commSnap = await db.collection('referrals').where('orderId', '==', latestOrder.id).get();
    if (commSnap.empty) {
