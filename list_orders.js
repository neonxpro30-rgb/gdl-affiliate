
const admin = require('firebase-admin');

const serviceAccount = {
    projectId: "gdl-database-b32f4",
    clientEmail: "firebase-adminsdk-fbsvc@gdl-database-b32f4.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
};

if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert(serviceAccount)
    });
}

const db = admin.firestore();

async function listRecentOrders() {
    console.log('Fetching recent SUCCESS orders (Client-side Sort)...');

    // Fetch all success orders (no ordering in query to avoid index)
    const snapshot = await db.collection('orders')
        .where('status', '==', 'SUCCESS')
        .get();

    if (snapshot.empty) {
        console.log('No recent success orders found.');
        return;
    }

    // Sort in memory
    const orders = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    orders.sort((a, b) => {
        const dateA = new Date(a.updatedAt).getTime();
        const dateB = new Date(b.updatedAt).getTime();
        return dateB - dateA; // Descending
    });

    // Take top 10
    const recentOrders = orders.slice(0, 10);

    // Also fetch referrals to check if they have commission
    const referralsSnapshot = await db.collection('referrals').get();
    const existingReferrals = referralsSnapshot.docs.reduce((acc, doc) => {
        const data = doc.data();
        if (data.orderId) acc[data.orderId] = true;
        return acc;
    }, {});

    recentOrders.forEach(data => {
        const hasCommission = existingReferrals[data.id] ? '✅ YES' : '❌ NO';
        console.log(`Order ID: ${data.id} | Commission: ${hasCommission}`);
        console.log(`  User: ${data.userId} | Amount: ${data.amount}`);
        console.log(`  Date: ${data.updatedAt}`);
        console.log('---');
    });
}

listRecentOrders();
