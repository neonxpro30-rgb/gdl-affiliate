const admin = require('firebase-admin');

if (!admin.apps.length) {
    const serviceAccount = {
        projectId: "gdl-database-b32f4",
        clientEmail: "firebase-adminsdk-fbsvc@gdl-database-b32f4.iam.gserviceaccount.com",
        privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
    };

    admin.initializeApp({
        credential: admin.credential.cert(serviceAccount),
    });
}

const db = admin.firestore();

async function createMissingCommission() {
    console.log('--- Creating Commission for "com test" user ---\n');

    // User and Referrer details from previous check
    const referredUserId = 'hOCOBjSVcmnIyNH4UgW4';  // com test
    const referrerId = 'KgvQCIZGzPW5SpXPknDV';      // Priyanshu

    // Check if commission already exists
    const existingRef = await db.collection('referrals')
        .where('referredUserId', '==', referredUserId)
        .where('type', '==', 'DIRECT')
        .limit(1)
        .get();

    if (!existingRef.empty) {
        console.log('⚠️ Commission already exists!');
        existingRef.forEach(doc => {
            const ref = doc.data();
            console.log('   Amount:', ref.amount);
            console.log('   Status:', ref.status);
        });
        process.exit(0);
    }

    // Create commission entry for Silicon Demo
    console.log('Creating ₹17 commission for Priyanshu...');

    await db.collection('referrals').add({
        referrerId: referrerId,                    // Priyanshu gets the commission
        referredUserId: referredUserId,            // com test who bought
        amount: 17,                                // Silicon fixed commission
        type: 'DIRECT',
        status: 'PENDING',
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
        notes: 'Backfilled - Silicon Demo purchase by com test'
    });

    console.log('✅ Commission created successfully!');
    console.log('   Referrer: Priyanshu (KgvQCIZGzPW5SpXPknDV)');
    console.log('   Amount: ₹17');
    console.log('   Status: PENDING');

    process.exit(0);
}

createMissingCommission().catch(console.error);
