const admin = require('firebase-admin');

if (!admin.apps.length) {
    const serviceAccount = {
        projectId: "gdl-database-b32f4",
        clientEmail: "firebase-adminsdk-fbsvc@gdl-database-b32f4.iam.gserviceaccount.com",
        privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
    };

    admin.initializeApp({
        credential: admin.credential.cert(serviceAccount),
    });
}

const db = admin.firestore();

async function checkSiliconCommissions() {
    console.log('--- Checking Silicon Package Orders & Commissions ---\n');

    // 1. Find all orders with Silicon package
    const ordersSnapshot = await db.collection('orders')
        .where('status', '==', 'SUCCESS')
        .get();

    const siliconOrders = ordersSnapshot.docs.filter(doc => {
        const data = doc.data();
        const productInfo = data.paymentData?.productinfo || '';
        const packageId = data.packageId || '';
        return productInfo.toLowerCase().includes('silicon') ||
            packageId.toLowerCase().includes('silicon');
    });

    console.log(`Found ${siliconOrders.length} Silicon orders:\n`);

    for (const orderDoc of siliconOrders) {
        const order = orderDoc.data();
        console.log('--- Order ---');
        console.log('Order ID:', orderDoc.id);
        console.log('User ID:', order.userId);
        console.log('Amount:', order.amount);
        console.log('Package ID:', order.packageId);
        console.log('Product Info:', order.paymentData?.productinfo);
        console.log('Created At:', order.createdAt);

        // Find user who made this order
        if (order.userId) {
            const userDoc = await db.collection('users').doc(order.userId).get();
            if (userDoc.exists) {
                const userData = userDoc.data();
                console.log('User Name:', userData.name);
                console.log('User Referrer ID:', userData.referrerId);

                // Check if commission was created for this referral
                const referralsSnapshot = await db.collection('referrals')
                    .where('referredUserId', '==', order.userId)
                    .get();

                if (referralsSnapshot.empty) {
                    console.log('⚠️ NO COMMISSION FOUND for this user!');
                } else {
                    referralsSnapshot.forEach(doc => {
                        const ref = doc.data();
                        console.log(`✅ Commission: ₹${ref.amount} | Type: ${ref.type} | Status: ${ref.status}`);
                    });
                }
            }
        }
        console.log('\n----------------------------\n');
    }

    // Also check all recent referrals
    console.log('\n--- Last 10 Referrals ---\n');
    const recentReferrals = await db.collection('referrals')
        .orderBy('createdAt', 'desc')
        .limit(10)
        .get();

    recentReferrals.forEach(doc => {
        const ref = doc.data();
        console.log(`Amount: ₹${ref.amount} | Type: ${ref.type} | Status: ${ref.status} | Date: ${ref.createdAt}`);
    });

    process.exit(0);
}

checkSiliconCommissions().catch(console.error);
