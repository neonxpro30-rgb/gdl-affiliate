
const admin = require('firebase-admin');

const serviceAccount = {
    projectId: "gdl-database-b32f4",
    clientEmail: "firebase-adminsdk-fbsvc@gdl-database-b32f4.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
};

if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert(serviceAccount)
    });
}
const db = admin.firestore();

async function debugOptimized() {
    console.log('Fetching Latest Order...');

    // 1. Fetch Latest Order (Optimized with limit)
    const ordersSnap = await db.collection('orders')
        .orderBy('createdAt', 'desc')
        .limit(1)
        .get();

    if (ordersSnap.empty) {
        console.log('No orders found at all.');
        return;
    }

    const latestOrder = ordersSnap.docs[0].data();
    latestOrder.id = ordersSnap.docs[0].id;

    console.log(`\nLatest Order Found:`);
    console.log(`- ID: ${latestOrder.id}`);
    console.log(`- Status: ${latestOrder.status}`);
    console.log(`- Amount: ${latestOrder.amount}`);
    console.log(`- User ID: ${latestOrder.userId}`);
    console.log(`- Date: ${latestOrder.createdAt}`);

    // 2. Fetch User
    const userDoc = await db.collection('users').doc(latestOrder.userId).get();
    if (!userDoc.exists) {
        console.log(`\n❌ User ${latestOrder.userId} NOT FOUND in DB!`);
        return;
    }
    const userData = userDoc.data();
    console.log(`\nBuyer Details:`);
    console.log(`- Name: ${userData.name}`);
    console.log(`- Referrer ID: ${userData.referrerId || 'NONE (⚠️ No Referrer Linked!)'}`);

    // 3. Fetch Referrer if exists
    if (userData.referrerId) {
        const refDoc = await db.collection('users').doc(userData.referrerId).get();
        if (refDoc.exists) {
            console.log(`- Referrer: ${refDoc.data().name} (${refDoc.data().referralCode})`);
        } else {
            console.log(`- Referrer ID ${userData.referrerId} exists in User Doc, but Referrer User Doc NOT FOUND!`);
        }
    }

    // 4. Fetch Commissions for this Order
    const commSnap = await db.collection('referrals')
        .where('orderId', '==', latestOrder.id)
        .get();

    if (commSnap.empty) {
        // Fallback: Check by referredUserId for recent commissions
        const recentCommSnap = await db.collection('referrals')
            .where('referredUserId', '==', latestOrder.userId)
            .limit(5)
            .get();

        if (recentCommSnap.empty) {
            console.log(`\n❌ NO COMMISSION found for this order.`);
        } else {
            console.log(`\n⚠️ Commission found for user but maybe link missing?`);
            recentCommSnap.docs.forEach(doc => {
                console.log(`- Comm ID: ${doc.id} | Amount: ${doc.data().amount} | OrderID: ${doc.data().orderId}`);
            });
        }
    } else {
        console.log(`\n✅ Commissions Found:`);
        commSnap.docs.forEach(doc => {
            const data = doc.data();
            console.log(`- Type: ${data.type} | Amount: ${data.amount} | To: ${data.referrerId}`);
        });
    }
}

debugOptimized();
