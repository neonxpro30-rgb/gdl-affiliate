const admin = require('firebase-admin');

if (!admin.apps.length) {
    const serviceAccount = {
        projectId: "gdl-database-b32f4",
        clientEmail: "firebase-adminsdk-fbsvc@gdl-database-b32f4.iam.gserviceaccount.com",
        privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
    };

    admin.initializeApp({
        credential: admin.credential.cert(serviceAccount),
    });
}

const db = admin.firestore();

async function fixSiliconCommissions() {
    console.log('--- Fixing Silicon ₹13 -> ₹17 Commissions ---\n');

    // Find all referrals with amount = 13 (wrongly calculated Silicon)
    const referralsSnapshot = await db.collection('referrals')
        .where('amount', '==', 13)
        .get();

    console.log(`Found ${referralsSnapshot.docs.length} referrals with ₹13 amount\n`);

    for (const doc of referralsSnapshot.docs) {
        const refData = doc.data();
        console.log('--- Referral ID:', doc.id);
        console.log('   Current Amount: ₹', refData.amount);
        console.log('   Referred User ID:', refData.referredUserId);
        console.log('   Type:', refData.type);
        console.log('   Status:', refData.status);

        // Check if this is from a Silicon order (amount = 19)
        const ordersSnapshot = await db.collection('orders')
            .where('userId', '==', refData.referredUserId)
            .where('status', '==', 'SUCCESS')
            .where('amount', '==', 19)
            .get();

        if (!ordersSnapshot.empty) {
            console.log('   ✅ Confirmed Silicon order found!');
            console.log('   🔧 Updating commission from ₹13 to ₹17...');

            await db.collection('referrals').doc(doc.id).update({
                amount: 17,
                updatedAt: new Date().toISOString(),
                notes: 'Corrected from ₹13 to ₹17 (Silicon fix)'
            });

            console.log('   ✅ Updated successfully!\n');
        } else {
            console.log('   ⚠️ Not a Silicon order, skipping\n');
        }
    }

    console.log('\n--- Done! ---');
    process.exit(0);
}

fixSiliconCommissions().catch(console.error);
