
const admin = require('firebase-admin');

// Hardcoded credentials (same as before)
const serviceAccount = {
    projectId: "gdl-database-b32f4",
    clientEmail: "firebase-adminsdk-fbsvc@gdl-database-b32f4.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
};

if (!admin.apps.length) {
    admin.initializeApp({
        credential: admin.credential.cert(serviceAccount)
    });
}

const db = admin.firestore();

async function deleteCollection(collectionPath, dateField, start, end) {
    console.log(`Deleting from ${collectionPath}...`);

    // Manual filtering because of composite index issues
    const snapshot = await db.collection(collectionPath).get();

    if (snapshot.empty) {
        console.log(`No documents in ${collectionPath}`);
        return;
    }

    const batch = db.batch();
    let count = 0;

    snapshot.docs.forEach(doc => {
        const data = doc.data();
        if (data[dateField]) {
            const date = new Date(data[dateField]);
            if (date >= start && date <= end) {
                batch.delete(doc.ref);
                count++;
            }
        }
    });

    if (count > 0) {
        await batch.commit();
        console.log(`Deleted ${count} documents from ${collectionPath}`);
    } else {
        console.log(`No documents found in range for ${collectionPath}`);
    }
}

async function resetDecemberStats() {
    const start = new Date('2025-12-01T00:00:00.000Z');
    const end = new Date('2025-12-31T23:59:59.999Z');

    console.log(`Resetting stats for range: ${start.toISOString()} to ${end.toISOString()}`);

    await deleteCollection('orders', 'createdAt', start, end);
    await deleteCollection('referrals', 'createdAt', start, end);

    console.log('December stats reset complete.');
}

resetDecemberStats();
