const admin = require('firebase-admin');

if (!admin.apps.length) {
    const serviceAccount = {
        projectId: "gdl-database-b32f4",
        clientEmail: "firebase-adminsdk-fbsvc@gdl-database-b32f4.iam.gserviceaccount.com",
        privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
    };

    admin.initializeApp({
        credential: admin.credential.cert(serviceAccount),
    });
}

const db = admin.firestore();

async function activateUsersWithSuccessOrders() {
    console.log('--- Activating users who have SUCCESS orders but are inactive ---\n');

    // Get all orders with SUCCESS status
    const ordersSnapshot = await db.collection('orders')
        .where('status', '==', 'SUCCESS')
        .get();

    console.log(`Found ${ordersSnapshot.docs.length} SUCCESS orders\n`);

    let activatedCount = 0;

    for (const doc of ordersSnapshot.docs) {
        const order = doc.data();
        const userId = order.userId;

        if (!userId) continue;

        // Get user
        const userDoc = await db.collection('users').doc(userId).get();
        if (!userDoc.exists) continue;

        const userData = userDoc.data();

        // If user is inactive but has SUCCESS order, activate them
        if (userData.isActive === false) {
            console.log(`Activating: ${userData.name} (${userData.email})`);

            await db.collection('users').doc(userId).update({
                isActive: true,
                updatedAt: new Date().toISOString()
            });

            activatedCount++;
        }
    }

    console.log(`\n✅ Activated ${activatedCount} users`);
    process.exit(0);
}

activateUsersWithSuccessOrders().catch(console.error);
