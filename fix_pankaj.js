const admin = require('firebase-admin');

if (!admin.apps.length) {
    const serviceAccount = {
        projectId: "gdl-database-b32f4",
        clientEmail: "firebase-adminsdk-fbsvc@gdl-database-b32f4.iam.gserviceaccount.com",
        privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
    };

    admin.initializeApp({
        credential: admin.credential.cert(serviceAccount),
    });
}

const db = admin.firestore();

async function fixPankajPayment() {
    console.log('--- Fixing Pankaj\'s Payment ---\n');

    const userId = '8z6qhGcXAiBKewI6oTKg';
    const orderId = 'UZD2gSvMRkQILYrfTCGS';

    // 1. Update Order to SUCCESS
    console.log('1. Updating order status to SUCCESS...');
    await db.collection('orders').doc(orderId).update({
        status: 'SUCCESS',
        transactionId: 'manual_fix_' + Date.now(),
        updatedAt: new Date().toISOString(),
        paymentData: {
            method: 'payu',
            fixedManually: true,
            reason: 'HTTP 405 error during PayU callback',
            date: new Date().toISOString()
        }
    });
    console.log('   ✅ Order updated to SUCCESS\n');

    // 2. Activate User
    console.log('2. Activating user...');
    await db.collection('users').doc(userId).update({
        isActive: true,
        updatedAt: new Date().toISOString()
    });
    console.log('   ✅ User activated\n');

    // 3. Get user's referrer to create commission
    console.log('3. Checking for referrer...');
    const userDoc = await db.collection('users').doc(userId).get();
    const userData = userDoc.data();

    if (userData.referrerId) {
        console.log('   Found referrer:', userData.referrerId);

        // Check if commission already exists
        const existingRef = await db.collection('referrals')
            .where('referredUserId', '==', userId)
            .where('type', '==', 'DIRECT')
            .limit(1)
            .get();

        if (existingRef.empty) {
            console.log('   Creating commission (₹17 for Silicon)...');
            await db.collection('referrals').add({
                referrerId: userData.referrerId,
                referredUserId: userId,
                amount: 17, // Silicon fixed commission
                type: 'DIRECT',
                status: 'PENDING',
                packageMismatch: false,
                createdAt: new Date().toISOString(),
                updatedAt: new Date().toISOString(),
                notes: 'Backfilled - Payment fixed after HTTP 405 error'
            });
            console.log('   ✅ Commission created\n');
        } else {
            console.log('   ⚠️ Commission already exists\n');
        }
    } else {
        console.log('   No referrer found\n');
    }

    console.log('--- Done! Pankaj can now login ---');
    process.exit(0);
}

fixPankajPayment().catch(console.error);
